...
// Runtime migration interface
export interface Migration {
  version: number;
  up: (db: DatabaseSync) => void;
}

// Import JSON migration files using the modern standard
import migration001Data from "./001-initial-tables.json" with { type: "json" };
import migration002Data from "./002-changes-with-type.json" with { type: "json" };
import migration003Data from "./003-added-removed-tables.json" with { type: "json" };
import migration004Data from "./004-api-check-table.json" with { type: "json" };
import migration005Data from "./005-api-check-status.json" with { type: "json" };

// Convert JSON migrations to runtime migrations
function createMigrationFromJSON(jsonMigration: JSONMigration): Migration {
  return {
    version: jsonMigration.version,
    up: (db: DatabaseSync) => {
      console.log(`  Running: ${jsonMigration.description}`);
      for (const statement of jsonMigration.statements) {
...
// Build migrations array from JSON data
